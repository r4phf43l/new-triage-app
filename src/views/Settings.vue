    ...
    <div class="columns">
      ...
      <div class="column">
        ...
        <form @submit.prevent="uploadJSON" v-if="tab==='bkp_json'">
          ...
          <div class="field is-grouped is-grouped-right">
            <div class="control">
              <button type="button" class="button is-primary is-large ml-2" v-on:click.self="defaultJSON" v-if="getURL">
                {{ 'settings.btn.default'|trans }} &nbsp;
                <span class="icon is-small">
                  <i class="fa fa-server"></i>
                </span>
              </button>
              <button type="button" class="button is-primary is-large ml-2" v-on:click.self="genJSON">
                {{ 'settings.btn.download'|trans }} &nbsp;
                <span class="icon is-small">
                  <i class="fa fa-cloud-download"></i>
                </span>
              </button>
              <button type="submit" class="button is-primary is-large ml-2">
    ...
    methods: {
      ...
      uploadJSON () {
        this.$swal({
          ...
          text: 'This will upload and Apply all your settings from your JSON.',
          icon: 'info',
          buttons: true,
          dangerMode: true
        }).then((willDelete) => {
      ...
      genJSON () {
        ...
        a.download = 'Triage-Settings.json'
        a.href = window.URL.createObjectURL(blob)
        a.dataset.downloadurl = ['text/json', a.download, a.href].join(':')
        e.initEvent('click', true, false, window, 0, 0, 0, 0, 0, false, false, false, false, 0, null)
        a.dispatchEvent(e)
        this.$swal('Download JSON', 'All your settings has been downloaded', 'success')
      },
      async defaultJSON () {
        let self = this
